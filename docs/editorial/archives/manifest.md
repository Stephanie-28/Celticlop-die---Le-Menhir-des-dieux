# Manifest du corpus Archives

## Légende

- **Texte long** : `FULL`, `SUMMARY_ONLY`, `MISSING`.
- **Doublon** : `UNIQUE`, `DUPLICATE`, `PARTIAL_OVERLAP`.
- `FULL_TEXT_MISSING` est inscrit dans les fichiers sources de chaque article lorsque seul un résumé est disponible.

## Savoirs officiels

| Titre | Source | Type actuel | Texte long | Doublon | Destination future | Action |
|---|---|---|---|---|---|---|
| La Quête du Graal Celte | PDF VII.1 | SAVOIR_OFFICIEL | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| Les Cycles Arthuriens et leurs Racines Celtiques | PDF VII.2 | SAVOIR_OFFICIEL | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| La Femme dans la Société Celtique | PDF VII.3 | SAVOIR_OFFICIEL | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| Les Druides — Mythes et Réalités | PDF VII.4 | SAVOIR_OFFICIEL | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| La Mort et l'Au-delà dans la Pensée Celtique | PDF VII.5 | SAVOIR_OFFICIEL | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| Les Vikings et les Celtes — Rencontres et Influences | PDF VII.6 | SAVOIR_OFFICIEL | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| L'Héritage Celtique dans la Culture Moderne | PDF VII.7 | SAVOIR_OFFICIEL | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| Les Fomoriens — Les Ennemis des Dieux | PDF VII.8 | SAVOIR_OFFICIEL | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| La Souveraineté Sacrée — Le Roi et la Terre | PDF VII.9 | SAVOIR_OFFICIEL | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| Les Celtes et Rome — Conquête et Résistance | PDF VII.10 | SAVOIR_OFFICIEL | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| La Transmission Orale — Comment les Mythes Ont Survécu | PDF VII.11 | SAVOIR_OFFICIEL | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| Le Renouveau Celtique — De la Renaissance à Aujourd'hui | PDF VII.12 | SAVOIR_OFFICIEL | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |

## Dernières Découvertes

| Titre | Source | Type actuel | Texte long | Doublon | Destination future | Action |
|---|---|---|---|---|---|---|
| Le Chant des Chênes | PDF V.1 | SAVOIR_DECOUVERTE | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| L'Oracle de Brigid | PDF V.2 | SAVOIR_DECOUVERTE | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| Grimoire des Herbes de Lune | PDF V.3 | SAVOIR_DECOUVERTE | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| Géométrie des Entrelacs | PDF V.4 | SAVOIR_DECOUVERTE | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| L'Art de la Prophétie Druidique | PDF V.5 | SAVOIR_DECOUVERTE | SUMMARY_ONLY | UNIQUE | Archives du Druide | Garder distinct du dossier Prophéties |
| Les Fêtes Sacrées du Calendrier Celtique | PDF V.6 | SAVOIR_DECOUVERTE | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| L'Écriture Ogham Décodée | PDF V.7 | SAVOIR_DECOUVERTE | SUMMARY_ONLY | PARTIAL_OVERLAP | Archives du Druide | Garder distinct du dossier Ogham |
| Les Quatre Villes Mythiques du Nord | PDF V.8 | SAVOIR_DECOUVERTE | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| Le Code d'Honneur des Fianna | PDF V.9 | SAVOIR_DECOUVERTE | SUMMARY_ONLY | DUPLICATE | Archives du Druide | Doublon brut signalé ; conserver occurrence canonique |
| Les Trois Fonctions Sacrées | PDF V.10 | SAVOIR_DECOUVERTE | SUMMARY_ONLY | DUPLICATE | Archives du Druide | Doublon brut signalé ; conserver occurrence canonique |
| Les Portes de l'Autre Monde | PDF V.11 | SAVOIR_DECOUVERTE | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| Le Langage Secret des Arbres | PDF V.12 | SAVOIR_DECOUVERTE | SUMMARY_ONLY | PARTIAL_OVERLAP | Archives du Druide | Garder distinct du dossier Ogham |
| Les Triades Celtiques | PDF V.13 | SAVOIR_DECOUVERTE | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| L'Art de la Forge Druidique | PDF V.14 | SAVOIR_DECOUVERTE | SUMMARY_ONLY | DUPLICATE | Archives du Druide | Doublon brut signalé ; conserver occurrence canonique |
| Les Nombres Sacrés | PDF V.15 | SAVOIR_DECOUVERTE | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| La Musique des Sphères Celtiques | PDF V.16 | SAVOIR_DECOUVERTE | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| Les Rites de Passage Celtiques | PDF V.17 | SAVOIR_DECOUVERTE | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| La Médecine des Druides | PDF V.18 | SAVOIR_DECOUVERTE | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| Les Inscriptions Sacrées de Gaule | PDF V.19 | SAVOIR_DECOUVERTE | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |
| L'Astronomie Druidique | PDF V.20 | SAVOIR_DECOUVERTE | SUMMARY_ONLY | UNIQUE | Archives du Druide | Conserver résumé ; rechercher texte long |

Les trois statuts `DUPLICATE` ci-dessus proviennent du signalement documentaire fourni. Une seule occurrence est matériellement présente dans le PDF accessible ; les autres fichiers bruts n'étant pas disponibles, aucune comparaison mot à mot supplémentaire n'est possible.

## Dossiers et Sagesses

| Titre | Source | Type actuel | Texte long | Doublon | Destination future | Action |
|---|---|---|---|---|---|---|
| Parchemins Anciens | PDF VI.1 | DOSSIER_ARCHIVES | FULL | UNIQUE | Savoirs Préservés | Porte d'entrée éditoriale |
| Alphabet Ogham | PDF VI.2 | DOSSIER_ARCHIVES | FULL | PARTIAL_OVERLAP | Savoirs Préservés | Dossier général / vitrine |
| Secrets d'Avalon | PDF VI.3 | DOSSIER_ARCHIVES | FULL | UNIQUE | Savoirs Préservés | Porte d'entrée éditoriale |
| Prophéties | PDF VI.4 | DOSSIER_ARCHIVES | FULL | UNIQUE | Savoirs Préservés | Garder distinct de l'étude sur l'art prophétique |
| Sagesse Druidique | PDF X + structure validée | DOSSIER_ARCHIVES | FULL | UNIQUE | Savoirs Préservés | Porte vers le corpus des Sagesses |
| Phrases de sagesse druidique | PDF X | SAGESSE | FULL | UNIQUE | Source documentaire unique | Arbitrer le total annoncé 70 / matériel 72 avant intégration |

## Chroniques

| Titre | Source | Type actuel | Texte long | Doublon | Destination future | Action |
|---|---|---|---|---|---|---|
| La Bataille de Mag Tuired | PDF I.1 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Le Rêve d'Aengus Og | PDF I.1 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| La Trahison de Blodeuwedd | PDF I.1 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| La Poursuite de Gwion Bach | PDF I.1 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Le Voyage de Bran | PDF I.1 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| La Naissance de Lugh | PDF I.1 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Manannan et l'Île des Pommes | PDF I.1 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Brigit et la Flamme Éternelle | PDF I.1 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| La Vengeance d'Arianrhod | PDF I.1 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Le Chaudron de Ceridwen | PDF I.1 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| La Malédiction de Macha | PDF I.2 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Cuchulainn et la Morrigan | PDF I.2 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| La Razzia des Vaches de Cooley | PDF I.2 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Le Combat du Gué | PDF I.2 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| La Mort de Cuchulainn | PDF I.2 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Le Festin de Bricriu | PDF I.2 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| La Décapitation du Guerrier | PDF I.2 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| L'Enfance de Cuchulainn | PDF I.2 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Le Roi Conchobar et Deirdre | PDF I.2 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| La Folie de Suibhne | PDF I.2 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Finn Mac Cumhaill et le Saumon de la Connaissance | PDF I.3 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| La Chasse du Sanglier de Ben Bulben | PDF I.3 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Diarmuid et Gráinne | PDF I.3 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| La Naissance de Finn | PDF I.3 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Les Fianna et la Forêt Sacrée | PDF I.3 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Oisín au Pays de la Jeunesse | PDF I.3 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| La Bataille de Gabhra | PDF I.3 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Le Chien Bran de Finn | PDF I.3 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| La Tour de Conan | PDF I.3 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Finn et le Géant Caledoine | PDF I.3 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Nuada et le Bras d'Argent | PDF I.4 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| La Pierre de Fal et le Premier Roi | PDF I.4 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Le Festin de Tara | PDF I.4 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| La Chute des Tuatha Dé Danann | PDF I.4 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Le Roi Conchobar et la Branche Rouge | PDF I.4 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| La Haute Royauté de Brian Boru | PDF I.4 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Cormac Mac Airt et la Coupe de Vérité | PDF I.4 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| La Malédiction des Rois | PDF I.4 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Niall aux Neuf Otages | PDF I.4 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |
| Le Crépuscule des Dieux | PDF I.4 | CHRONIQUE | FULL | UNIQUE | Chroniques Mythiques | Inventaire uniquement |

## Anciennes structures

| Titre | Source | Type actuel | Texte long | Doublon | Destination future | Action |
|---|---|---|---|---|---|---|
| Sites Sacrés dans Archives | PDF II.1 | LEGACY_SOURCE_ONLY | FULL | UNIQUE | Source seulement | Ne pas importer dans Archives |
| Symboles Sacrés dans Archives | PDF II.2 | LEGACY_SOURCE_ONLY | FULL | UNIQUE | Reliques Célestes | Utiliser le modèle Symbole actuel |
| Bestiaire Mythologique dans Archives | PDF II.3 | LEGACY_SOURCE_ONLY | FULL | UNIQUE | Reliques Célestes | Utiliser le modèle Animal actuel |
| Ancien Chemin de l'Initiation | PDF III | LEGACY_SOURCE_ONLY | FULL | UNIQUE | Chemin actuel | Ne pas modifier |
| Anciennes Reliques Célestes | PDF IV | LEGACY_SOURCE_ONLY | FULL | UNIQUE | Reliques actuelles | Ne pas restaurer |
| Ancien Focus du Mois | PDF VI | LEGACY_SOURCE_ONLY | FULL | UNIQUE | Focus séparé + dossiers | Ne pas réintroduire son architecture |
| Ancien Sanctuaire | PDF VIII | LEGACY_SOURCE_ONLY | FULL | UNIQUE | Mon Sanctuaire actuel | Ne pas modifier |
| Ancien Profil de l'Initié | PDF IX | LEGACY_SOURCE_ONLY | FULL | UNIQUE | Profil actuel | Ne pas modifier |

## Décision d'interface pour l'étape d'intégration

`UI_ARCHIVES_SAVOIRS: CAROUSEL_HORIZONTAL`

- La section « Savoirs Préservés » présente une rangée horizontale glissante de cinq portes d'entrée, dans cet ordre : Parchemins Anciens, Alphabet Ogham, Secrets d'Avalon, Prophéties, Sagesse Druidique.
- Sur mobile, une carte principale et une amorce de la suivante restent visibles ; seul le carrousel défile horizontalement.
- Sur tablette et desktop, plusieurs cartes peuvent être visibles, tout en conservant le défilement horizontal si nécessaire.
- Le comportement doit privilégier `overflow-x-auto`, `flex-nowrap` et le scroll snap, sans dépendance JavaScript lourde.
- Le lien « Tout voir » reste distinct du carrousel et mène à la bibliothèque complète des Savoirs.
- La valeur `GRID` est explicitement rejetée pour cette section.
- Cette décision est documentaire uniquement : aucun template n'est modifié pendant l'étape de sécurisation.

## Contrôles et divergences

- **12/12** Savoirs officiels inventoriés ; 12 résumés, 12 corps longs manquants.
- **20/20** Découvertes inventoriées ; 20 résumés, 20 corps longs manquants.
- **5/5** dossiers éditoriaux identifiés.
- **40/40** Chroniques identifiées et laissées hors de `Savoir`.
- Les trois PDF accessibles sont des copies exactes (`DUPLICATE` au niveau fichier).
- Doublons bruts signalés : L'Art de la Forge Druidique, Le Code d'Honneur des Fianna, Les Trois Fonctions Sacrées. Les copies brutes ne sont pas accessibles pour comparaison.
- Chevauchements partiels : Alphabet Ogham / L'Écriture Ogham Décodée / Le Langage Secret des Arbres.
- Le PDF annonce **70** Sagesses mais contient **72 phrases uniques** réparties 12 + 10 + 10 + 10 + 10 + 10 + 10. Les 72 sont conservées ; supprimer deux phrases violerait la règle de conservation absolue. Arbitrage éditorial requis avant intégration.
