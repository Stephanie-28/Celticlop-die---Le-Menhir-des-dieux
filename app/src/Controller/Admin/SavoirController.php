...
final class SavoirController extends AbstractController
{
    ...
    public function index(SavoirRepository $repository): Response { return $this->render('admin/content/index.html.twig', ['items' => $repository->findBy([], ['createdAt' => 'DESC']), 'title' => 'Savoirs préservés', 'description' => 'Administrer les savoirs conservés dans les archives.', 'userStory' => 'US49 — Gérer les savoirs préservés', 'newRoute' => 'app_admin_savoir_new', 'newLabel' => 'Ajouter un Savoir préservé', 'editRoute' => 'app_admin_savoir_edit', 'deleteRoute' => 'app_admin_savoir_delete', 'deleteTokenPrefix' => 'delete-savoir-', 'displayProperty' => 'title', 'subtitleProperty' => 'summary', 'countProperty' => null]); }

    #[Route('/nouveau', name: 'new', methods: ['GET', 'POST'])]
    public function new(Request $request, EntityManagerInterface $em, SavoirRepository $repository): Response { $savoir = (new Savoir())->setCreatedAt(new \DateTimeImmutable())->setIsFocus(false); return $this->save($savoir, $request, $em, $repository, true); }

    #[Route('/{id}/modifier', name: 'edit', requirements: ['id' => '\d+'], methods: ['GET', 'POST'])]
    public function edit(Savoir $savoir, Request $request, EntityManagerInterface $em, SavoirRepository $repository): Response { return $this->save($savoir, $request, $em, $repository, false); }
